...
7. **TEST (recommended for webhook/chat/form workflows)**: Execute the workflow in test mode
   - `npx --yes n8nac test <id>`: Detects the trigger type, calls the test-mode webhook URL, and reports the result.
...
- **inspect then test after push for webhook/chat/form workflows**: Run `npx --yes n8nac test-plan <id>` first, then `npx --yes n8nac test <id>`. A Class A error is not a bug — tell the user. A runtime-state issue is also not a code bug — fix the state/arming problem, not the workflow code. A Class B error is fixable — iterate.
